...
interface TaskCardProps {
  ...
  onEdit: (task: Task) => void; // Added onEdit prop here
}
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center mt-2 text-sm text-slate-500">
           ...
           <span className={isOverdue ? "text-red-500 font-medium" : isToday ? "text-green-600 font-bold" : ""}>
             {task.deadline} {isToday && "(Today)"} {isOverdue}
           </span>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-lg text-slate-800">{currentMonth} {currentYear}</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-2 text-center text-sm mb-2">
        {['S','M','T','W','T','F','S'].map((d, i) => ( 
           <div key={i} className="text-slate-400 text-xs font-bold">{d}</div> 
        ))}
      </div>
        ...
        if (result.success) {
          setTasks(prev => prev.map(t => 
            t.id === editingId ? { 
              ...t, 
              ...formData, 
              type: formData.type as any // Fixes the type error
            } : t
          ));
        }
      } else {
